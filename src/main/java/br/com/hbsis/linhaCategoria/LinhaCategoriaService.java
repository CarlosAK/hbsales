...
public class LinhaCategoriaService {
    ...
    public LinhaCategoriaDTO save(LinhaCategoriaDTO linhaCategoriaDTO) {
        ...
        String codigoCompleto = validarCodigo(codigoRecebido);
        codigoCompleto = codigoCompleto.toUpperCase();

        linhaCategoria.setCodLinhaCategoria(codigoCompleto);
    ...
    public LinhaCategoriaDTO update(LinhaCategoriaDTO linhaCategoriaDTO, Long id) {
        ...
        if (linhaCategoriaExistenteOptional.isPresent()) {
            ...
            LOGGER.info("Atualizando linha categoria... id:[{}]", linhaCategoriaExistente.getIdLinhaCategoria());
    ...
    public void csvToLinhaCategoriaExport(HttpServletResponse response) throws Exception {
        ...
        for (LinhaCategoria linha : iLinhaCategoriaRepository.findAll()) {
            icsvWriter.writeNext(new String[]{linha.getCodLinhaCategoria(), linha.getNomeLinhaCategoria(), linha.getCategoria().getCodCategoria(), linha.getCategoria().getNomeCategoria()}
    ...
    private void validate(LinhaCategoriaDTO linhaCategoriaDTO) {
        ...
        if (linhaCategoriaDTO == null) {
            throw new IllegalArgumentException("LinhaCategoriaDTO não deve ser nulo/vazio");

        }

        if (StringUtils.isEmpty(linhaCategoriaDTO.getCodLinhaCategoria())) {
            throw new IllegalArgumentException("Codigo da linha categoria não deve ser nulo/vazio");

        }

        if (StringUtils.isEmpty(linhaCategoriaDTO.getNomeLinhaCategoria())) {
            throw new IllegalArgumentException("Nome linha categoria não dve ser nulo/vazio");
